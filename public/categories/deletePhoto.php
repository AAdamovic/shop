...
$pageTitle = "Delete " . $category['name'] . " photo";
...
if (isset($formData["click"]) && ($formData["click"] == "Delete" || $formData["click"] == "Cancel")) {
	...
	if ($formData["click"] == "Delete") {
        ...
        $id = deleteCategoryPhoto($id, $connection);        
        if($id){
            $_SESSION['systemMessage'] = "Category  " . $category['name'] . " image was deleted sucessfully";
            header("Location: /categories/list.php");
            die();
        }
	}
...
require_once '../../view/categories/deletePhotoView.php';
