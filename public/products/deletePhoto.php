...
$pageTitle = "Delete " . $product['title'] . " photo";
...
if (isset($formData["click"]) && ($formData["click"] == "Delete" || $formData["click"] == "Cancel")) {
	...
	if ($formData["click"] == "Delete") {
        ...
        $id = deleteProductPhoto($id, $connection);        
        if($id){
            $_SESSION['systemMessage'] = "Product  " . $product['title'] . " image was deleted sucessfully";
            header("Location: /products/list.php");
            die();
        }
	}
...
require_once '../../view/products/deletePhotoView.php';
