...
if (isset($formData["click"]) && $formData["click"] == "Save") {
	...
	if (isset($formData["title"])) {
		...
		if ($formData["title"] === "") {
            ...
            if(mb_strlen($formData['title'] > 100)){
                ...
                if(!checkProductNameIsUnique($formData['title'], $connection)){
	...
	if (empty($formErrors)) {
        ...
        if (empty($formErrors)) {
            // save product
            $id = createProduct($formData, $image, $connection);
            if($id){
                $product = getProduct($id, $connection);
                $_SESSION['systemMessage'] = "Product: " . $product['title'] . " was created sucessfully";
...
require_once '../../view/products/createView.php';
